		...
		<div>
			<div className="mb-3 flex items-center gap-3">
				...
				<span className="text-sm text-gray-600">
					{completed}/{total}
					{running > 0 && (
						<span className="text-blue-600 ml-1">({running} running)</span>
					)}
				</span>
			...
			<div className="overflow-x-auto">
				<table className="text-sm border-collapse">
					<thead>
						<tr>
							<th className="px-3 py-1 text-left text-gray-500 font-normal">
								Scenario
							</th>
							{models.map((m) => (
								<th key={m} className="px-3 py-1 text-center font-medium">
									{m}
								</th>
							))}
						</tr>
								...
								{models.map((m) => {
									...
									return (
										<td key={m} className="px-3 py-1 text-center">
											<span
												className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${cellColors[status]}`}
											>
												{status}
											</span>
										</td>
